...
- 文章可见性、评论、回复和点赞
- 默认关闭的单文章 Blog Agent：访客可在文章页提问，并基于当前文章及其可选私有材料包跳转到原文引用
...
Blog Agent 始终以一篇 Blog 为唯一数据与安全边界。机器人只出现在 `published + public` 且包含 Markdown 的文章页；服务端根据 URL slug 重新读取当前文章，请求体只能包含一个 1～500 字符的问题。默认上下文是当前 Markdown。需要回答实现细节时，发布 Skill 可以为新文章附加一份 Git `HEAD` 审核过的代码、文档、数据或图片说明快照；检索 SQL 仍先锁定当前 `blog_id + package_hash`，不会跨文章搜索。

文章包仅由 admin/editor 认证接口写入，source 内容和 embedding 不进入匿名 Blog API。包模式是 create-only，单个 source、总大小和 chunk 数都有硬上限；索引未 ready 或文章内容/hash 变化时，Payload 会拒绝公开发布。访客查询不会实时连接本地 Codex 或 Git 仓库，embedding 失败时只在当前包内退回 lexical 检索，包不可用时退回当前 Markdown。

运行时使用 PostgreSQL 保存匿名 hash 用量、生成事件、答案缓存和当前文章的私有索引，不保存原始 IP 或原始问题。模型与 embedding API Key 仅存在于服务端环境变量，入口与生成使用两个默认关闭的开关，并有持久化配额、并发限制、超时、缓存和紧急关闭能力。
