...
锁文件审计是 CI 门禁；扫描步骤固定 npm 11 audit 客户端并显式使用官方 registry，避免 npm 10 已退役的 quick endpoint 和本地镜像缺少 audit API。若审计仍报告 low/moderate 上游问题，应核对 Payload 与 Next.js 的兼容版本；没有无破坏性升级路径时，不应使用 `npm audit fix --force` 越过框架兼容范围。
