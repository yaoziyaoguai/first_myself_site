...
describe('Slug trim hook - beforeValidate', () => {
  ...
  describe('Blog collection slug trim', () => {
    ...
    it('should trim leading and trailing spaces', () => {
      ...
      const result = hook?.({ data } as unknown as MockHookData)
  ...
  describe('Hook consistency across collections', () => {
    ...
    it('All hooks should have the same slug-trim behavior', () => {
      ...
      const blogResult = getBlogHook()?.({ data: blogData } as unknown as MockHookData)
      const postsResult = getPostsHook()?.({ data: postsData } as unknown as MockHookData)
      const projectsResult = getProjectsHook()?.({ data: projectsData } as unknown as MockHookData)
    ...
    it('should all skip non-string slug values', () => {
      ...
      expect(blogHook?.({ data: numberSlug } as unknown as MockHookData).slug).toBe(123)
      expect(postsHook?.({ data: numberSlug } as unknown as MockHookData).slug).toBe(123)
      expect(projectsHook?.({ data: numberSlug } as unknown as MockHookData).slug).toBe(123)
    })
  })
