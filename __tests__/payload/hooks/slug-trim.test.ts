...
describe('Slug trim hook - beforeValidate', () => {
  ...
  describe('Hook consistency across collections', () => {
    ...
    it('All hooks should have the same slug-trim behavior', () => {
      ...
      const blogResult = getBlogHook()?.({ data: blogData } as any)
      const postsResult = getPostsHook()?.({ data: postsData } as any)
      const projectsResult = getProjectsHook()?.({ data: projectsData } as any)
    ...
    it('should all skip non-string slug values', () => {
      ...
      expect(blogHook?.({ data: numberSlug } as any).slug).toBe(123)
      expect(postsHook?.({ data: numberSlug } as any).slug).toBe(123)
      expect(projectsHook?.({ data: numberSlug } as any).slug).toBe(123)
    })
  })
