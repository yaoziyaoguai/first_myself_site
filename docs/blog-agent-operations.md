...
访问记录不保存完整 IP，只保存 IPv4 `/24` 或 IPv6 `/64` 的脱敏网段。通过 Payload 验证的 admin/editor 请求会签发一个签名、HttpOnly、`SameSite=Lax` 的站长设备标记；后续同一浏览器即使退出后台，统计请求仍会被标为站长访问。该标记只用于排除统计，不具备认证或授权能力，客户端自行伪造的字段或无效签名不会被信任。后台总览还会排除已出现站长记录的同一访客指纹，并由迁移回溯修正已有记录。访问记录与脱敏后的 Agent 问题保留 30 天。
